...
class PartitionManager {
    ...
    set data (d) {
        this.items = (d.items || []).map(item => {
            ...
            // migration boolean → integer pour les patterns sauvegardés avant v2.0.0
    ...
    onProgress ( xhr ) {
        if ( xhr.lengthComputable ) {
            ...
            if (percentComplete < 100) {
                ...
                elem.innerHTML = '<i class="icon-sliders"></i> Guitar Lab <span class="app-version">2.0.0</span>';
            }
        }
    }
    onError ( xhr ) {
...
class PanelReperes extends UXPanel {
    ...
    mountContent (container) {
        ...
        INTERVALS.forEach(({ key, label, name }) => {
            const icname = key === 'r' ? 'itR' : 'it' + key.charAt(0).toUpperCase() + key.slice(1);
            const cell = document.createElement('div');
            cell.className = 'reperes-cell';
            const icEl = ic(icname);
            icEl.classList.add('reperes-cell-icon');
            const labelEl = document.createElement('span');
            labelEl.className = 'reperes-cell-label';
            labelEl.textContent = label;
            const nameEl = document.createElement('span');
            nameEl.className = 'reperes-cell-name';
            nameEl.textContent = name;
            cell.append(icEl, labelEl, nameEl);
...
class Application {
  constructor (onReady = () => {}, guitardef = null) {
        ...
        this.appstamp.innerHTML = '<i class="icon-sliders"></i> Guitar Lab <span class="app-version">2.0.0</span>';
