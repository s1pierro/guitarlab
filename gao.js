...
class PartitionManager {
    ...
    set data (d) {
        this.items = (d.items || []).map(item => {
            ...
            // migration boolean → integer pour les patterns sauvegardés avant v1.9.4.9
    ...
    onProgress ( xhr ) {
        if ( xhr.lengthComputable ) {
            ...
            if (percentComplete < 100) {
                ...
                elem.innerHTML = '<i class="icon-sliders"></i> Guitar Lab <span class="app-version">1.9.4.9</span>';
            }
        }
    }
    onError ( xhr ) {
...
class Application {
  constructor (onReady = () => {}, guitardef = null) {
        ...
        this.appstamp.innerHTML = '<i class="icon-sliders"></i> Guitar Lab <span class="app-version">1.9.4.9</span>';
        ...
        const applyPluckPos = () => {
            ...
            if (p) {
                requestAnimationFrame(() => {
                    ...
                    const left = Math.min(Math.max(0, p.rx * window.innerWidth),  window.innerWidth  - r.width);
                    const top  = Math.min(Math.max(0, p.ry * window.innerHeight), window.innerHeight - r.height);
